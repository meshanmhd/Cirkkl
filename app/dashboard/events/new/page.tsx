...
export default function NewEventPage() {
  ...
  const [form, setForm] = useState({
    ...
    venue: "", addressLine1: "", addressLine2: "", city: "", state: "", pincode: "", country: "India",
    meetingLink: "", platform: "",
    price: "free", capacity: "", approvalRequired: "false", registrationDeadline: "", registrationEndTime: "",
    dressCode: "", minAge: "", languages: "", parking: false, food: false, wifi: false,
  ...
  const handleSubmit = async (isDraft = false) => {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from("events").insert({
        ...
        venue: form.venue,
        location: [form.addressLine1, form.addressLine2, form.city].filter(Boolean).join(", "),
        city: form.city,
        state: form.state,
            ...
            {(form.locationType === "physical" || form.locationType === "hybrid") && (
              <div className="flex flex-col gap-4">
                ...
                <FormInput label="Address Line 2">
                  <input className={inputCls} placeholder="Apartment, suite, building (optional)" value={form.addressLine2} onChange={e => set("addressLine2", e.target.value)} />
                </FormInput>
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <FormInput label="Pincode">
                    <input className={inputCls} placeholder="000000" value={form.pincode} onChange={e => set("pincode", e.target.value)} />
                  </FormInput>
                </div>
              </div>
            )}
